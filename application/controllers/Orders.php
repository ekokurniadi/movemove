...
class Orders extends MY_Controller {
    ...
    function kode()
    {
             $this->db->select('RIGHT(orders.kode_order,2) as kode_order', FALSE);
    ...
    public function create_action() 
    {
		...
		'total_customer_bayar' => $this->input->post('total_customer_bayar',TRUE),
		'biaya_layanan' => $this->input->post('biaya_layanan',TRUE),
		'ongkos_driver' => $this->input->post('ongkos_driver',TRUE),
	...
	public function blastOrdersAgain($kode_order){
		...
		$data=array();
			...
			$driverOnSameArea = $this->db->query("select * from driver where status='1' and black_list ='0' and saldo >= 20000 and status_online ='1' and confirm ='1' and not EXISTS(select a.driver from orders a where a.status_pengantaran ='process')");
	...
	public function blastOrders($kode_order){
		date_default_timezone_set("Asia/Jakarta");
	...
	$this->form_validation->set_rules('bantuan_customer', 'bantuan customer', 'trim|numeric');
	$this->form_validation->set_rules('barang_customer', 'barang customer', 'trim|required');
	$this->form_validation->set_rules('bantuan_driver', 'bantuan driver', 'trim|numeric');
	$this->form_validation->set_rules('bantuan_kenek', 'bantuan kenek', 'trim|numeric');
	$this->form_validation->set_rules('biaya_tol', 'biaya tol', 'trim|numeric');
	$this->form_validation->set_rules('biaya_overload', 'biaya overload', 'trim|numeric');
	$this->form_validation->set_rules('waktu_tunggu', 'waktu tunggu', 'trim|numeric');
